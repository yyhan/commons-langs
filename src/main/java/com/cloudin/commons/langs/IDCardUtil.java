...
public class IDCardUtil {
	...
	public static Date getBirthDate(String cardNumber) {
		try {
			return new Date(createBirthDateParser().parse(getBirthDayPart(cardNumber)).getTime());
		} catch (Exception e) {
			throw new RuntimeException("身份证的出生日期无效");
		}
	}

	public boolean isMale(String cardNumer) {
		return 1 == getGenderCode(cardNumer);
	}

	public boolean isFemal(String cardNumer) {
		return false == isMale(cardNumer);
	}
	...
	private static void checkIfValid(String cardNumer) {
		if (false == validate(cardNumer)) {
	...
	public static void main(String[] args) {
		System.out.println(IDCardUtil.validate("320525198511080590"));
	}
}